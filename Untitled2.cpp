...
struct facturas{
	...
	struct lista_articulos_ lista_articulos;

} factura[15];

char c;
int i, j, opc, ran, contadorfacturas, artFacturas[15], w, x, y, z, cuantas, desde, menus, coincidencias;
...
char Adjetivos[14][12]={"salado, ","grande, ","juvenil, ","infantil, ","seco, ","caro, ","fuerte, ","ácido, ","limpio, ","agradable, ","flexible, ","esponjoso, ","dulce, ","mucho, "};
	...
	switch(opc){
		...
		case 0:
			...
			for(i=0; i<datosf.nro_facturas; i++){
				...
				factura[i].clientes.CI=rand()%25000000+10000000;
				ran=rand()%15;
				factura[i].clientes.direccion[90]=Direcciones[ran][90];
				
				factura[i].clientes.numero_telefono[12]={48,52,50,52,48,48,48,48,48,48,48};
				
				printf("%s",factura[i].clientes.numero_telefono);
				printf("\n\n");
				system("pause");
				
				for(z=4; z<11; z++){
					
					factura[i].clientes.numero_telefono[z]=rand()%10+48;
				}
				
				datosf.items=rand()%15;
				artFacturas[i]=datosf.items;
				factura[i].lista_articulos.cant=rand()%30;
				ran=rand()%7;
				
				for(z=0;z<ran;z++){
				
					j=rand()%14;
					strcpy(factura[z].lista_articulos.descripcion,Adjetivos[j]);
				}
				
				factura[i].lista_articulos.precio_unitario=rand()%1000;
				contadorfacturas++;
		...
		case 1:
			...
			for(i=contadorfacturas; i<datosf.nro_facturas; i++){
				...
				printf("Ingrese el numero de celular del cliente (sin el primer cero por favor): ");
				scanf("%s",&factura[i].clientes.numero_telefono);
...
void Imprimir_Facturas(int cuantas,int desde){
	...
	for(i=x-1; i<15 && i<(x-1+w); i++){
		...
		printf("\nNumero telefonico: %s",factura[i].clientes.numero_telefono);
	...
	for(j=0; j<artFacturas[i]; j++){
		...
		factura[j].lista_articulos.importe=factura[j].lista_articulos.precio_unitario*factura[j].lista_articulos.cant;
		
		ran=strlen(factura[j].lista_articulos.descripcion);
		printf("    %d    | ",factura[j].lista_articulos.cant);
		
		ran=0;				
		for(y=0; y<((77-ran)/2); y++){printf(" ");}
		printf("%s",factura[j].lista_articulos.descripcion);
		for(y=0; y<((77-ran)/2); y++){printf(" ");}
		
		printf(" | %d | %d ",factura[j].lista_articulos.precio_unitario,factura[j].lista_articulos.importe);
		
		factura[i].lista_articulos.subtotal+=factura[j].lista_articulos.importe;
		printf("\n");
	}
		factura[i].lista_articulos.total=factura[i].lista_articulos.subtotal+(factura[i].lista_articulos.subtotal*IVA);
		
		printf("\n");
		printf("                                                                                        |      Subtotal     %d\n",factura[i].lista_articulos.subtotal);
		printf("                                                                                        |      IVA (21%%)    %d\n\n",factura[i].lista_articulos.subtotal*IVA);
		printf("                                                                                        |       TOTAL       %d\n",factura[i].lista_articulos.total);
		printf("\n\n\n");
		system("pause");
	}
	return;
...
void Buscar(){
	...
	if(opc==1 || opc==4 || opc==5 || opc==7){
		
		printf("\nIngrese la cantidad que busca: ");
		scanf("%d",&buscarN);
	}
	
	if(opc==2 || opc==3 || opc==6){
	...
	switch(opc){
		...
		case 2:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(strcmp(buscarC,factura[i].clientes.direccion)){
		...
		case 3:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(strcmp(buscarC,factura[i].clientes.nombre)){
		...
		case 4:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(strcmp(buscarC,factura[i].clientes.numero_telefono)){
		...
		case 5:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(buscarN==factura[i].lista_articulos.cant){
		...
		case 6:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(strcmp(buscarC,factura[i].lista_articulos.descripcion)){
		...
		case 7:
			for(i=0; i<=contadorfacturas; i++){
				...
				if(buscarN==factura[i].lista_articulos.total){
