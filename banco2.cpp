...
Movimiento *CrearMovimiento(float modif_saldo, int nro_referencia, char concepto [20], char CI_beneficiario[8], char nombre_beneficiario[30], char nroCuenta_beneficiario[20], char banco_beneficiario[20], char tlf_beneficiario[11],int bool_);
...
void imprimir(Pila *pila){
	...
	Movimiento*c=(Movimiento*)malloc(sizeof(Movimiento));
		...
		printf("Fecha:%d/%d/%d \nHora %d:%d:%d\nreferencia %d\nConcepto: %s\nCedula del Beneficiario: %s\n Nombre del beneficiario: %s\n", c->dia,c->mes,c->anio,c->hora,c->min,c->seg,c->nro_referencia,c->concepto,c->CI_beneficiario,c->nombre_beneficiario);

		if(c->bool_){
			 printf("\nNumero de cuenta: %s",c->nroCuenta_beneficiario);
		}else{
			printf("\n telefono: %s ",c->tlf_beneficiario);
		}
		
		printf("\nBanco: %s",c->banco_beneficiario);
...
int fecha(int parametro)
{
	...
	return 1;
		...
		if(c->bool_){
			 printf("Numero de cuenta: %s",c->nroCuenta_beneficiario);
		}else{
			printf("\n telefono: %s\n ",c->tlf_beneficiario);
		}
		
		printf("Banco: %s\n",c->banco_beneficiario);
